...
namespace AwaitableCircularBuffer
{
    ...
    {
        private AwaitableCircularBuffer<double> _buffer;

        [OneTimeSetUp]
        public void Setup()
        {
            _buffer = new AwaitableCircularBuffer<double>(10);
        }
        ...
        public void Test_SetAndGet()
        {
            ...
            var notifyEvent = _buffer.GetNotifyEvent(4);

            // Write the first 2 items, event should not be set
            _buffer.Put(testArray.Take(2).ToArray());
            Assert.IsFalse(notifyEvent.IsSet);

            // Write 2 more items, event should be set
            _buffer.Put(testArray.Skip(2).Take(2).ToArray());
            ...
            _buffer.Put(testArray.Take(9).ToArray());
            Assert.IsFalse(notifyEvent.IsSet);
            Assert.AreEqual(1, _buffer.LostChunks);
        }
    }
}
